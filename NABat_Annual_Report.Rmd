...
#Load Packages
list.of.packages <- c("data.table", "leaflet", "tidyverse", "lunar", "zoo", "colortools", "lubridate", "camtrapR", "circular", "RColorBrewer", "Cairo", "viridis", "knitr", "sf","osmdata", "ggspatial", "ggmap","gridExtra")
...
sta$Land.Use.Type <- sta$Land.Cover %>% recode("Coniferous Forest" = "Forest-conifer", "Broadleaf Forest" = "Forest-deciduous", 
                                               "Mixed Forest" = "Forest-mixed", "Exposed Land" = "Barren Land")
...
LANO.WWT <- volancy_overlap.fn(Classification=class.list[6,])
...
```{r WNS spread png map, out.width="0.3\\linewidth", include=TRUE, fig.align="center", fig.cap="White-nose syndrome spread up to 2020", echo=FALSE}
knitr::include_graphics("./Input/WNS_spread_map_2021-03-15.png")
```

##### Figure 1. The spread of White-Nose Syndrome up to the spring of 2020; downloaded from https://www.whitenosesyndrome.org/where-is-wns.
...
```{r nightly survey effort NP, echo=F, fig.height=7, fig.width=6}
...
```{r mean nightly call by NR, echo=F, fig.height=7, fig.width=6, warning=FALSE}
...
```{r mean nightly call by LUT, echo=F, fig.height=7, fig.width=6, warning=FALSE}
...
```{r mean nightly call by GRTS, echo=F, fig.height=7, fig.width=6, warning=FALSE}
...
```{r sp hist NR, echo=F, fig.height=7, fig.width=6}
...
```{r sp hist LUT, echo=F, fig.height=7, fig.width=6}
...
```{r mean species call by year and volancy, echo=F, fig.height=7, fig.width=6, warning=FALSE}
