...
dat_summary <- dat_summary %>% mutate(Timep = coalesce(Time.temp1p, Time.temp2p, Time.temp3p, Time.temp4p, Time.temp5p,Time.temp6p, Time.temp7p)) %>% 
  select(-c(Time.temp1, Time.temp2, Time.temp3, Time.temp4, Time.temp5, Time.temp6,
            Time.temp1p, Time.temp2p, Time.temp3p, Time.temp4p, Time.temp5p,Time.temp6p,Time.temp7p))
...
eff <- read.csv(paste0("Input/NABat_Deployment_Data_",Year_interest,".csv"), header=T) #%>% mutate(Survey.Start.Time = ymd(Survey.Start.Time), Survey.End.Time = ymd(Survey.End.Time))
...
# as.data.frame(eff %>% ungroup() %>% group_by(Deployment.ID) %>% count(GRTS.Cell.ID)) # 34+ NABat cells surveyed in 2023
...
# eff %>% filter(Location.Name %in% no.data$Location.Name) %>% select(Location.Name, Orig.Name, Detector.Failure.Details) %>% as_tibble() %>% print(n=30)
...
# 1 "9626_NE_01"   Muskeg 2       "equipment malfunction" 
# 2 "148871_NE_01" WoS            "no data"       
...
eff %>% filter(Deployment.ID=="2023") %>% count(GRTS.Cell.ID) # 34 (+ up to 28 more) GRTS cells surveyed
nrow(eff %>% filter(Deployment.ID=="2023")) # 89 sites

# sta.sites <- sta %>% filter(X2023==1) %>% select(Location.Name); nrow(sta.sites) #63
sta.sites <- sta %>% select(Location.Name); nrow(sta.sites) #63
# eff.sites <- eff %>% filter(Deployment.ID=="2023") %>% select(Location.Name, Orig.Name); nrow(eff.sites) #89
eff.sites <- eff %>% select(Location.Name, Orig.Name); nrow(eff.sites) #89
...
dist.mat <- read.csv("DistanceMatrixTable_2023.csv", header=T, row.names=1)
dim(dist.mat)
...
# didn't input this year (2021, 2022, or 2023), will use first and last date bat recorded instead
...
length(setdiff(unique(sta$Location.Name), unique(dat_summary$Location.Name))) # 3
...
sta %>% count(Location.Name) #62 stations (3 with malfunctions or no files sent)
sta %>% count(GRTS.Cell.ID) # 31 (without Bayne data)
...
sta %>% filter(!Location.Name %in% missing.data.names$Location.Name) %>% count(Location.Name) # 59 stations with data
sta %>% filter(!Location.Name %in% missing.data.names$Location.Name) %>% count(GRTS.Cell.ID) # 30 grid cells with data (without Bayne data)
