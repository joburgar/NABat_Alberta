...
dat_summary <- dat_summary %>% mutate(Timep = coalesce(Time.temp1p, Time.temp2p, Time.temp3p, Time.temp4p, Time.temp5p,Time.temp6p)) %>% 
  select(-c(Time.temp1, Time.temp2, Time.temp3, Time.temp4, Time.temp5, Time.temp6,
            Time.temp1p, Time.temp2p, Time.temp3p, Time.temp4p, Time.temp5p,Time.temp6p))
...
as.data.frame(eff %>% ungroup() %>% group_by(Deployment.ID) %>% count(GRTS.Cell.ID)) # 119 NABat cells surveyed in 2022
...
### NO DATA (i.e., equipment malfunction) for 14 sites and no data sent for 1 site
# Location.Name Orig.Name          Detector.Failure.Details
#  1 3667_NW_01    Wabasca - Unit 1   "Detector malfunction for entire survey"
#  2 9626_NE_01    Muskeg 2           "file issues"
#  3 27242_NE_01   Tonquin            "Did not send files on recording cycle - only daily recordings Jun 15 - Jul 15"
#  4 45447_NW_01   45447_NW_01        "Just picked up false calls and noise."
#  5 45447_SE_02   45447_SE_02        "Did not even pick up any noise. Equipment not working properly."
#  6 45447_SE_03   45447_SE_03        ""
#  7 99719_NE_01   99719_NE_01        ""
#  8 160135_SE_01  160135_SE_01       "For some reason picked up basically NO calls. Not sure why, perhaps too far from any structures and treed habitats. "
#  9 216730_NW_02  BP-S-E0-1-134N     ""
# 10 224135_SE_02  Lost River - creek "no data provided - equipment malfunction"
# 11 231018_NW_01  Isaac Creek        "Extensive water damage. Recorder filled with water and significantly corroded. Did not turn on.
# 12 264178_SE_02  BP-W-5-4_BAT       ""
# 13 267463_NE_01  Brule Tunnel Road  "Unit failed"
# 14 276408_NW_02  BP-P-E0-1-23N      "no data provided - not in retrieval excel file"
# 15 323194_SW_01  BP-RD-1-107N       "2 \"calls\" picked up - malfunction?" 
...
eff %>% filter(Deployment.ID=="2022") %>% count(GRTS.Cell.ID) # 119 GRTS cells surveyed
nrow(eff %>% filter(Deployment.ID=="2022")) # 173 sites

# sta.sites <- sta %>% filter(X2022==1) %>% select(Location.Name); nrow(sta.sites) #173
sta.sites <- sta %>% select(Location.Name); nrow(sta.sites) #173
# eff.sites <- eff %>% filter(Deployment.ID=="2022") %>% select(Location.Name, Orig.Name); nrow(eff.sites) #173
eff.sites <- eff %>% select(Location.Name, Orig.Name); nrow(eff.sites) #173
...
dist.mat <- read.csv("DistanceMatrixTable_2022.csv", header=T, row.names=1)
dim(dist.mat)
...
# dat_summary$Deployment.ID <- case_when(dat_summary$Orig.Name=="30403" ~ "2021",
#                                      dat_summary$Orig.Name=="38595" ~ "2021",
#                                      dat_summary$Orig.Name=="42883" ~ "2021",
#                                      TRUE ~ as.character(dat_summary$Deployment.ID))
...
length(setdiff(unique(sta$Location.Name), unique(dat_summary$Location.Name))) # 15
...
sta %>% count(Location.Name) #173 stations (15 with malfunctions or no files sent)
sta %>% count(GRTS.Cell.ID) # 119

# 4c) Do you have all data represented in the deployment data? If yes, the value should be 0
length(setdiff(unique(sta$Location.Name), unique(dat_summary$Location.Name))) # 15
missing.dat.sta <- left_join(sta, dat_summary, by="Location.Name") %>% dplyr::select(Location.Name, Orig_Name, Orig.Name)
missing.dat.sta[is.na(missing.dat.sta$Orig.Name),] # will show missing deployment data covariates

# sta %>% filter(Location.Name=="323194_SW_01")

sta %>% filter(!Location.Name %in% missing.data.names$Location.Name) %>% count(Location.Name) # 158 stations with data
sta %>% filter(!Location.Name %in% missing.data.names$Location.Name) %>% count(GRTS.Cell.ID) # 116 grid cells with data
...
sum(ARUs.by.LUC$n) # 136
sum(ARUs.by.NP$n) # 37
sum(ARUs.by.NR$n) # 173
sum(ARUs.by.LUT$n) # 173
