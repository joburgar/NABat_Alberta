...
sta$Land.Unit.Code <- recode(sta$Land.Unit.Code, "BNP"="BANP", "JNP"="JANP","LPR"="LOPR","LAR"="LOAR",
...
dist.mat <- read.csv("Input/DistanceMatrixTable_2021.csv", header=T, row.names=1)
...
sum(is.na(dat_summary$Timep)) / nrow(dat_summary) # almost 20% having issues with filename (only 3 % in 2019; 1.6% in 2021)
...
sum(ARUs.by.LUC$n) # 26
sum(ARUs.by.NP$n) # 35
sum(ARUs.by.NR$n) # 61
sum(ARUs.by.LUT$n) # 61
