...
dat_summary %>% filter(Orig.Name=="OnefourPlayground")
...
dat_summary %>% filter(time.keep=="cut") %>% summarize(mean(Timep), max(Timep))
...
# as.data.frame(eff %>% ungroup() %>% group_by(Deployment.ID) %>% count(GRTS.Cell.ID)) #
nrow(eff) #123 stations surveyed in 2023 (does not include mobile)
eff %>% count(Detector.Failure.Details) # 115 good, 8 without data
...
names(dat_summary)
eff %>% filter(Location.Name==no.data$Location.Name[1])
...
# 4 238163_NW_01  BAT-23-F-7084 ""                                
# 5 313693_SW_01  BAT-23-W-500  "no data"                         
# 6 323478_SE_02  BAT-23-W-502  "no data"                         
# 7 335882_SW_01  BAT-23-W-571  "Have metadata, but no recordings"     
...
nrow(eff %>% filter(Deployment.ID=="2023")) # 123 sites

sta.sites <- sta %>% select(Location.Name); nrow(sta.sites) #123

eff.sites <- eff %>% select(Location.Name, Orig.Name); nrow(eff.sites) #123
...
length(setdiff(unique(eff$Location.Name), unique(sta$Location.Name))) # 1
...
missing.data.names <- missing.dat.sta[is.na(missing.dat.sta$Orig.Name),]  # will show missing count data (may just not have data for certain years) # same 7 as above

sta %>% filter(Location.Name %in% missing.data.names$Location.Name) %>% select(Location.Name, Orig_Name)

sta %>% count(Location.Name) #123 stations (7 with malfunctions or no files sent)
sta %>% count(GRTS.Cell.ID) # 75 (74 GRTS in eff - missing metadata?)
...
sta %>% filter(!Location.Name %in% missing.data.names$Location.Name) %>% count(Location.Name) # 116 stations with data
sta %>% filter(!Location.Name %in% missing.data.names$Location.Name) %>% count(GRTS.Cell.ID) # 73 grid cells with data
