...
eff %>% count(Location.Name) #173 stations (15 with malfunctions or no files sent)
eff %>% count(GRTS.Cell.ID) # 119

# 4c) Do you have all data represented in the deployment data? If yes, the value should be 0
length(setdiff(unique(eff$Location.Name), unique(dat_summary$Location.Name))) # 15
missing.dat.eff <- left_join(eff, dat_summary, by="Location.Name") %>% dplyr::select(Location.Name, Orig.Name.x, Orig.Name.y)
missing.dat.eff[is.na(missing.dat.eff$Orig.Name.y),] # will show missing deployment data covariates


eff %>% filter(!Location.Name %in% missing.data.names$Location.Name) %>% count(Location.Name) # 158 stations with data
eff %>% filter(!Location.Name %in% missing.data.names$Location.Name) %>% count(GRTS.Cell.ID) # 116 grid cells with data
...
sum(total.effort$Diff) # 3367 survey nights
