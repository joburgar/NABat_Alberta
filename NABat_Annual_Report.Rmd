...
eff %>% filter(Detector.Failure.Details!="") %>% count(GRTS.Cell.ID)
...
calls.per.night.locn %>% ungroup() %>% filter(Call.Count>500)

calls.per.night.locn %>% filter(Location.Name=="134339_NW_01")
...
# mean(tmp1$nights_surveyed) #23
# se(tmp1$nights_surveyed) #3
...
call_count %>% group_by(Volancy) %>% summarise(Mean = mean(Count), SE = se(Count))
...
# GRTS.surveyed %>% ungroup() %>% summarise(mean(Count), min(Count), max(Count), se(Count), median(Count))
# GRTS.surveyed %>% arrange(Count) %>% tail()
# GRTS.surveyed %>% ungroup() %>% filter(Count <5) %>% count(GRTS.Cell.ID)
# GRTS.surveyed %>% ungroup() %>% filter(Count >100) %>% count(GRTS.Cell.ID)
# 
...
GRTS.Calls.Yr.Order <- GRTS.Calls.Yr %>% arrange(Land.Unit.Code, GRTS.Cell.ID) %>% 
  group_by(Land.Unit.Code) %>% count(GRTS.Cell.ID)

GRTS.Calls.Yr %>% group_by(Land.Unit.Code) %>% arrange(Land.Unit.Code)

summary(GRTS.Calls.Yr$Mean)
fcalls.GRTS <- GRTS.Calls.Yr %>% group_by(Land.Unit.Code) %>%
...
call_count %>% filter(Classification=="MYCA") %>% group_by(Land.Unit.Code)%>% summarise(sum(Count))
329/1827
...
Sp.Calls.Yr %>% arrange(Mean) %>% print(n=30)
