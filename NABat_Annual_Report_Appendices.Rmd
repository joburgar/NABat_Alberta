...
## NABat 2021 Report Appendix

``` {r map with inset, echo=F, warning=F, message=F}
  grid.newpage()
...
##### Figure 1. GRTS Cell `r params$GRTS.Cell.ID` (black square) with stations survyed in `r unique(dat_count$Year)` and inset map showing grid location compared to other grid cells (gray) surveyed in `r unique(dat_count$Year)` throughout Alberta. 
...
``` {r Figure 2, fig.height=6, fig.width=7, echo=F, warning=F, message=F}
###- Appendix Figure 2
  # only graph species / species groups with >100 calls
sp.to.use <- call_count.Sp %>% group_by(GRTS.Cell.ID, Classification,Year) %>% summarise(Sum = sum(Count))
  
  
  # group data for overall mean nightly bat calls by year with volancy
app.Calls <- call_count.Sp%>% group_by(GRTS.Cell.ID, Classification, Year) %>% summarise(Mean = mean(Count), SE = se(Count))
...
##### Figure 2. Mean (± 1 Standard Error) nightly bat calls for GRTS Cell `r params$GRTS.Cell.ID` in `r unique(dat_count$Year)`. 
  
``` {r Figure 3, fig.height=6, fig.width=7, echo=F, warning=F, message=F}
###- Appendix Figure 3
...
##### Figure 3. Total bat calls for GRTS Cell `r params$GRTS.Cell.ID` in `r unique(dat_count$Year)`. 
...
``` {r Table 3, echo=F, warning=F, message=F}
###- Appendix Table 3 - Nightly call counts
  ...
  potential.bat.sp <- c("EPFU", "EPFU-LANO", "LANO", "LACI", "LABO", "LABO-MYLU", "MYLU","MYCA", "MYCI", "MYEV","MYEV-MYSE", "MYSE", "MYVO", "Myotis 40k", "unknown","noise")
  actual.bat.sp <- names(Appendix.Table3)[3:ncol(Appendix.Table3)]
  not.detected.bat.sp <- potential.bat.sp[!(potential.bat.sp%in% actual.bat.sp)]

  # add in null columns (sp not detected)
  Appendix.Table3[not.detected.bat.sp] <- as.character(NA)
  
  # put in the same order for all appendices
