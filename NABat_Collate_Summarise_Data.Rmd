...
The majority (i.e., `r nrow(sta)-sum(ARUs.by.NP$n)` or `r round((nrow(sta)-sum(ARUs.by.NP$n))/nrow(sta)*100,0)`%) of bat survey locations were outside of National Parks. Of the `r sum(ARUs.by.NP$n)` (`r round(sum(ARUs.by.NP$n)/nrow(sta)*100,0)`%) survey lcoations within parks, `r sum(ARUs.by.NP[ARUs.by.NP$Land.Unit.Code %in% c("BNP","JNP", "WLNP"),]$n)` were part of the Mountain Park Network. National Parks deployed ARUs at an average of `r round(mean(ARUs.by.NP$n),1)` ± `r round(se(ARUs.by.NP$n),1)` locations; ranging from `r ARUs.by.NP[1,2]` locations within `r ARUs.by.NP[1,1]` to `r ARUs.by.NP[5,2]` locations within both `r ARUs.by.NP[5,1]` and `r ARUs.by.NP[4,1]`. Considering the area of each park, BNP and JNP had proportional survey representation, i.e., `r round(sta.NP %>% filter(Land.Unit.Code=="BNP") %>% summarise(sum(prop))*100,0)`% and `r round(sta.NP %>% filter(Land.Unit.Code=="JNP") %>% summarise(sum(prop))*100,0)`% of Alberta's park area and proportion of bat survey locations within the parks. In contrast, WLNP comprises only 1% of Alberta's National Park area but had `r round(sta.NP %>% filter(Land.Unit.Code=="WLNP") %>% summarise(sum(prop))*100,0)`% of park bat survey locations while WBNP comprises 71% of park area with only `r round(sta.NP %>% filter(Land.Unit.Code=="WLNP") %>% summarise(sum(prop))*100,0)`% of park bat survey locations. EINP is Alberta's smallest National Park, comprising not even 1% of Alberta's park area but with `r round(sta.NP %>% filter(Land.Unit.Code=="EINP") %>% summarise(sum(prop))*100,0)`% of the surveys. 
```{r map NP, echo=F}
...
mPA
```
Figure 2. Distribution of bat survey locations in/out of National Parks. 

Bat survey locations occurred within each of the six Natural Regions, with an average `r round(mean(ARUs.by.NR$n),1)` ± `r round(se(ARUs.by.NR$n),1)` of bat survey locations within each Natural Region. Most (i.e., `r ARUs.by.NR[1,2]` or  `r round(ARUs.by.NR[1,2]/nrow(sta)*100,0)`%) locations were within the `r ARUs.by.NR[1,1]` and only `r ARUs.by.NR[6,2]` (i.e., `r round(ARUs.by.NR[6,2]/nrow(sta)*100,0)`%) within the `r ARUs.by.NR[6,1]`. Considering the area of each Natural Region, the Boreal was undersubscribed in terms of bat survey locations (i.e., `r round(sta.NR %>% filter(Natural.Region=="Boreal") %>% summarise(sum(prop))*100,0)`% of the survey locations but comprising 58% of Alberta's land mass. The Parkland and Canadian Shield Natural Regions were proportionally represented (i.e, `r round(sta.NR %>% filter(Natural.Region=="Parkland") %>% summarise(sum(prop))*100,0)`% and `r round(sta.NR %>% filter(Natural.Region=="Canadian Shield") %>% summarise(sum(prop))*100,0)`%, respectively of Alberta's area and bat survey locations). In contrast, Foothills, Grassland and Rocky Mountain Natural Regions were proportionally overrepresented in terms of bat survey locations to area (`r round(sta.NR %>% filter(Natural.Region=="Foothills") %>% summarise(sum(prop))*100,0)`%, `r round(sta.NR %>% filter(Natural.Region=="Grassland") %>% summarise(sum(prop))*100,0)`%, and `r round(sta.NR %>% filter(Natural.Region=="Rocky Mountain") %>% summarise(sum(prop))*100,0)`, of locations compared to 10%, 14%, and 7% of Alberta's area, respectively).
...
LUT.hist
```
Figure 5. Representation of bat survey locations by Land Use Type in/out of National Parks (NP) 
...
Yr.hist
```
Figure 6. Annual bat survey effort by NABat grid cell in/out of National Parks (NP) 

# Nightly Acoustic Survey Effort (`r min(eff$Deployment.ID)` to `r max(eff$Deployment.ID)`)


```{r nightly survey effort NP, echo=F}
...
# julian day for July 10
yday("2020-07-10") # 192
yday("2020-01-01")# 192
as.Date("2020-01-01") + 181
yday("2020-09-01")
...
NightPA.hist
```
Figure 7. Number of bat survey locations sampled per night from `r min(eff$Deployment.ID)` to `r max(eff$Deployment.ID)` by year in/out of National Parks (NP).

```{r nightly survey effort NR, echo=F}
...
NightNR.hist
```
Figure 8. Number of bat survey locations sampled per night from `r min(eff$Deployment.ID)` to `r max(eff$Deployment.ID)` by year in each Natural Region.
