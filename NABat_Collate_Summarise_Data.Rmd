...
```{r Table 1 bat survey locations echo=F}
sta.count <- sta %>% count(PA, Natural.Region, Land.Unit.Code)
sta.count2 <- sta %>% group_by(PA, Natural.Region, Land.Unit.Code) %>% count(GRTS.Cell.ID)
sta.count2$GRTS.count <- 1
sta.count3 <- sta.count2 %>% group_by(PA, Natural.Region, Land.Unit.Code) %>% summarise(sum(GRTS.count))

sta.count$Count.Grid.Cells <- sta.count3[,4]
knitr::kable(sta.count,
      colnames=c("Protected Area", "Natural Region", "Land Unit", "Count Bat Survey Locations", "Count NABat Grid Cells"),
      align = "lllrr")
```
...
Yr.hist
```
Figure 6. Annual bat survey effort by NABat grid cell in/out of Protected Areas (PA) 

# Nightly Acoustic Survey Effort (`r min(eff$Deployment.ID)` to `r max(eff$Deployment.ID)`)


```{r nightly survey effort PA, echo=F}
dat_count$PA <- sta$PA[match(dat_count$Location.Name, sta$Location.Name,)]
...
calls.per.night.locn <- dat_count %>% group_by(Year,jDay, SurveyNight,PA, Natural.Region, Land.Unit.Code, Location.Name) %>%
...
NightPA.hist <- ggplot(data = calls.per.night.locn, aes(x = jDay, y = Night.Count, fill=PA)) + 
...
NightPA.hist
```
Figure 7. Number of bat survey locations sampled per night from `r min(eff$Deployment.ID)` to `r max(eff$Deployment.ID)` by year in/out of Protected Areas (PA).
