...
#Load Packages
list.of.packages <- c("data.table", "leaflet", "tidyverse", "lunar", "zoo", "colortools", "mapview", "fs", "lubridate", "camtrapR", "circular", "Hmsc", "corrplot", "plotrix", "RColorBrewer", "MuMIn", "Cairo", "viridis")
...
eff$Deployment.ID <- as.Date(eff$Deployment.ID)
# eff <- eff %>% filter(Deployment.ID==2020)
...
sta <- read.csv("Input/NABat_Station_Covariates.csv", header=T, na.string=c("","NA", "<NA>",-1)) %>%
  # dplyr::select(FID, Land.Unit.Code, GRTS.Cell.ID, Location.Name, Cardinal_D, Stn_Num, Orig.Name, NABat.Sample, Yrs.Surveyed, LC_type, NSRNAME, NRNAME,
...
sum(total.effort$Diff) # 3327 survey nights
```
...
#### Summary Statistics
All analyses were conducted in R (version 3.6.1; 2019), within RStudio (version 1.2.5033) using rmarkdwon (version 2.1; Allaire et al. 2020) for reproducible results. Unless otherwise noted, results are reported as the mean ± 1 SE. 
...
nrow(dat_sum_sub) # now down to 198840
...
locations.to.use <- sta[1:113,]$Location.Name
  
# Exclude EINP as no station covariates
dat <- dat %>% filter(Location.Name %in% locations.to.use)
dat %>% count(Location.Name) #111 stations, stations without covariates have been removed
...
sta2 <- sta %>% filter(Location.Name %in% locations.to.use) # now only 113 stations
nrow(sta2)

# modify the GIS covariates to suit the analysis
### Road type and distance
sta2 %>% count(Road.Type) # change road type to ordinal scale from 1 = no road to 8 = divided paved road
sta2$Road.Type <- as.character(sta2$Road.Type)

# if Road is within 100 m, keep as Road.Type.Ord but if >200 then change to 1, i.e., "No Road"
sta2$Road.Type <- case_when(sta2$Road.Distance > 200 ~ "No Road",
                               TRUE ~ as.character(sta2$Road.Type))
# recode to ordinal scale, also changing NA to No Road as it meant no road within 1000 m
sta2$Road.Type.Ord <- sta2$Road.Type %>% replace_na("No Road") %>%
...
max(sta2$Road.Distance, na.rm=T) # 759 m, so max category is 9 for roads > 800 and those with NA (none within 1000 m)
sta2$Road.Distance.Ord <- ifelse(sta2$Road.Distance<100, 1,
                                ifelse(sta2$Road.Distance<200, 2,
                                       ifelse(sta2$Road.Distance<300, 3,
                                              ifelse(sta2$Road.Distance<400, 4,
                                                     ifelse(sta2$Road.Distance<500, 5,
                                                            ifelse(sta2$Road.Distance<600, 6,
                                                                   ifelse(sta2$Road.Distance<700, 7,
                                                                          ifelse(sta2$Road.Distance<800, 8,
                                                                                 ifelse(sta2$Road.Distance>800,9, NA)))))))))

sta2$Road.Distance.Ord <- sta2$Road.Distance.Ord %>% replace_na(9)
sta2 %>% group_by(Road.Distance.Ord) %>% summarise(min(Road.Distance), max(Road.Distance))

### Water type and distance
sta2 %>% filter(is.na(Waterbody.Distance)) %>% count(Stream.Type)
...
sta2 %>% filter(Stream.Distance<200 | Waterbody.Distance<200) %>% group_by(Stream.Type, Waterbody.Type) %>%
  select(Location.Name, Stream.Distance, Waterbody.Distance, Stream.Type, Waterbody.Type)

sta2$Water.Distance <- with(sta2, pmin(Stream.Distance, Waterbody.Distance, na.rm=T))
# to remove NAs, will change this to ordinal scale later as actually >1000 but unknown
sta2$Water.Distance <- replace_na(sta2$Water.Distance, 1001)

sta2$Water.Type <- case_when(sta2$Waterbody.Distance < sta2$Stream.Distance ~ as.character(sta2$Waterbody.Type),
                            TRUE ~ as.character(sta2$Stream.Type))
as.data.frame(sta2 %>% group_by(Water.Type) %>% select(Waterbody.Type, Stream.Type, Waterbody.Distance, Stream.Distance, Water.Distance))

max(sta2$Water.Distance, na.rm=T) # 1001 m, so max category is 11 for water with 11 being >1000 m

# recode to ordinal scale, also changing NA to No Water as it meant no waterbody or stream within 1000 m
sta2$Water.Type.Ord <- sta2$Water.Type %>% replace_na("No Water") %>%
...
sta2 %>% group_by(Water.Type) %>% count(Water.Type.Ord)
hist(sta2$Water.Type.Ord)

sta2$Water.Distance.Ord <- ifelse(sta2$Water.Distance<100, 1,
                                ifelse(sta2$Water.Distance<200, 2,
                                       ifelse(sta2$Water.Distance<300, 3,
                                              ifelse(sta2$Water.Distance<400, 4,
                                                     ifelse(sta2$Water.Distance<500, 5,
                                                            ifelse(sta2$Water.Distance<600, 6,
                                                                   ifelse(sta2$Water.Distance<700, 7,
                                                                          ifelse(sta2$Water.Distance<800, 8,
                                                                                 ifelse(sta2$Water.Distance<900,9,
                                                                                        ifelse(sta2$Water.Distance<1000,10,11))))))))))

sta2 %>% group_by(Water.Distance.Ord) %>% summarise(min(Water.Distance), max(Water.Distance))
# hist(sta2$Water.Distance.Ord)

### Human Footprint type and distance
as.data.frame(sta2 %>% count(Human.Footprint.Type)) # consolidate to sublayer from ABMI HF 2014 metadata, similar as 2018 report
sta2$Human.Footprint.Distance <- replace_na(sta2$Human.Footprint.Distance, 1001)
sta2$Human.Footprint.Sublayer <- ifelse(sta2$Human.Footprint.Type %in%
                                       ...
                                       ifelse(sta2$Human.Footprint.Type %in% c("CROP","CULTIVATION_ABANDONED","ROUGH_PASTURE", "TAME_PASTURE"), "Agriculture",
                                              ifelse(sta2$Human.Footprint.Type %in% c("CLEARING-UNKNOWN", "GREENSPACE","RECREATION", "FACILITY-UNKNOWN"), "Greenspace" ,
                                                     ifelse(sta2$Human.Footprint.Type %in% c("HARVEST-AREA"), "Forestry",
                                                            ifelse(sta2$Human.Footprint.Type %in% c("TRAIL", "VEGETATED-EDGE-ROADS"), "Transportation",
                                                                   ifelse(sta2$Human.Footprint.Type %in% c("RURAL-RESIDENCE", "URBAN-RESIDENCE"), "Residence", NA))))))

sta2$Human.Footprint.Sublayer <- case_when(sta2$Human.Footprint.Distance > 200 ~ "No Human Footprint",
                            TRUE ~ as.character(sta2$Human.Footprint.Sublayer))

sta2 %>% count(Human.Footprint.Sublayer)
as.data.frame(sta2 %>% group_by(Human.Footprint.Sublayer) %>% count(Human.Footprint.Type))
summary(sta2$Human.Footprint.Distance) # 94 are within 100 m of site, so let's change to log for ordinal scale
sta2$Human.Footprint.Distance.log <- log(sta2$Human.Footprint.Distance+1)
summary(sta2$Human.Footprint.Distance.log) # now values range from 0-6.9
# recode distance to ordinal scale 1-10 using log transformed distance values
sta2$Human.Footprint.Distance.Ord <- ifelse(sta2$Human.Footprint.Distance.log<1, 1,
                                ifelse(sta2$Human.Footprint.Distance.log<2, 2,
                                       ifelse(sta2$Human.Footprint.Distance.log<3, 3,
                                              ifelse(sta2$Human.Footprint.Distance.log<4, 4,
                                                     ifelse(sta2$Human.Footprint.Distance.log<5, 5,
                                                            ifelse(sta2$Human.Footprint.Distance.log<6, 6,7))))))

sta2 %>% group_by(Human.Footprint.Distance.Ord) %>% summarise(min(Human.Footprint.Distance), max(Human.Footprint.Distance))

# use actual distance rather than ordinal scale, but replace 0 values with 1 and NA with 1001 for model convergence
hist(ifelse(sta2$Human.Footprint.Distance<1, 1, sta2$Human.Footprint.Distance))
hist(ifelse(sta2$Water.Distance<1, 1, sta2$Water.Distance))
hist(ifelse(sta2$Road.Distance<1,1, sta2$Road.Distance))

sta2$HF.Dist.HMSC <- ifelse(sta2$Human.Footprint.Distance<1, 1, sta2$Human.Footprint.Distance)
sta2$Water.Dist.HMSC <- ifelse(sta2$Water.Distance<1, 1, sta2$Water.Distance)
sta2$Road.Dist.HMSC <- ifelse(sta2$Road.Distance<1,1, sta2$Road.Distance)
sta2$Road.Dist.HMSC <- replace_na(sta2$Road.Dist.HMSC,1001)

# create index of road / water importance - multiply ordinal "type" by distance
# the larger the feature type the greater the influence
sta2$Water.Inf <- sta2$Water.Dist.HMSC*sta2$Water.Type.Ord
sta2$Road.Inf <- sta2$Road.Dist.HMSC*sta2$Road.Type.Ord
...
as.data.frame(weekly.max.env.cov %>% filter(is.na(Max.Temp)) %>% filter(is.na(Max.Hum)) %>% count(Location.Name.Year))
no.weather.df <- weekly.max.env.cov[is.na(weekly.max.env.cov$Max.Temp),]
no.weather.df$Location.Name <- substr(no.weather.df$Location.Name.Year, 1,  nchar(as.character(no.weather.df$Location.Name.Year))-5)
no.weather.df <- left_join(no.weather.df, sta%>% select(Location.Name, Orig.Name, GRTS.Cell.ID, Land.Unit.Code))
no.weather.df %>% group_by(Location.Name) %>% count(Location.Name.Year)
no.weather.df$Year <- word(no.weather.df$Location.Name.Year, 4, sep="_")

# will remove 100 survey weeks of data if want to include max temp and max hum
# will remove 37 stations as weather data limiting based on station
# for now keep weather out of analysis
...
# also have non-ordinal distances with 1 in place of 0 and 1001 for NA (as not included but so few may not matter)
# also created a weighted distance with weight being 'size' of feature on ordinal scale for water and road
...
###--- also remove weeks with <1 days of effort and <1 identified bat call
# for model convergence and to deal with sites with such low detections likely from malfunctioning equipment
week.obs %>% group_by(Location.Name,Year) %>% summarise(sum(Effort))
table(week.obs$Effort, week.obs$Year)
week.obs <- week.obs %>%
  rowwise() %>% 
  mutate(batcalls = sum(c(`EPFU`, `EPFU-LANO`, `LANO`, `LACI`, `LABO`, `LABO-MYLU`, `MYLU`, `Myotis 40k`)))

week.obs <- week.obs %>% filter(batcalls>1) %>% filter(Effort>1)
table(week.obs$Effort, week.obs$Year)
...
HMSC.stn.yr.week <- unique(week.obs$Location.Name_Yr_Week) # the data points for the HMSC analysis
nrow(week.obs)

# Make the Y Matrix
Y <- week.obs[c(6:13)]
Y.rownames <- week.obs$Location.Name_Yr_Week
...
nrow(sta.week)
sta.week <- left_join(sta.week, sta2 %>% select("Location.Name","NP","Natural.Region","Land.Unit.Code","Land.Use.Type","Latitude","Longitude","Road.Type.Ord","Road.Dist.HMSC","Water.Type.Ord","Water.Dist.HMSC","HF.Dist.HMSC"))

XData <- data.frame(sta.week)
nrow(XData)

XData$Location.Name_Yr_Week <- paste(XData$Location.Name, XData$Year, XData$Week,sep="_")
XData <- XData %>% filter(Location.Name_Yr_Week %in% HMSC.stn.yr.week)

#relevel categorical covariates
XData$Natural.Region <- droplevels(XData$Natural.Region)
...
XData$Road.Distance <- stdize(XData$Road.Dist.HMSC)
XData$Water.Type <- stdize(XData$Water.Type.Ord)
XData$Water.Distance <- stdize(XData$Water.Dist.HMSC)
XData$Human.Footprint.Distance <- stdize(XData$HF.Dist.HMSC)
...
# Predictors
XData.predictors <- c("Latitude","Road.Type","Road.Distance","Water.Type","Water.Distance","Human.Footprint.Distance")
tmp <- cor(XData[,c(XData.predictors)])
corrplot.mixed(tmp) # 0.83 negative correlation between Road.Distance and Human.Footprint.Distance
...
Stations were distributed across Alberta with a slight concentration in the south-western portion of the province (Figure 1). The majority (i.e., `r nrow(sta)-sum(ARUs.by.NP$n)` or `r round((nrow(sta)-sum(ARUs.by.NP$n))/nrow(sta)*100,0)`%) of stations were outside of National Parks. Of the `r sum(ARUs.by.NP$n)` (`r round(sum(ARUs.by.NP$n)/nrow(sta)*100,0)`%) stations within parks, `r sum(ARUs.by.NP[ARUs.by.NP$Land.Unit.Code %in% c("BNP","JNP", "WLNP"),]$n)` were part of the Mountain Park Network. On average, there were `r round(mean(ARUs.by.NP$n),1)` ± `r round(se(ARUs.by.NP$n),1)` stations within each National Park; ranging from `r ARUs.by.NP[1,2]` stations within `r ARUs.by.NP[1,1]` to `r ARUs.by.NP[5,2]` stations within both `r ARUs.by.NP[5,1]` and `r ARUs.by.NP[4,1]`. Considering the area of each National Park,JNP had proportional survey representation, i.e., `r round(sta.NP %>% filter(Land.Unit.Code=="JNP") %>% summarise(sum(prop))*100,0)`% of Alberta's park area and proportion of stations within the parks. In contrast, WLNP and EINP were over-represented, and WBNP was under-represented, in terms of number of stations to park area. 

Outside of National Parks, stations were within `r nrow(ARUs.by.LUC)` Land Units, with an average of `r round(mean(ARUs.by.LUC$n),1)` ± `r round(se(ARUs.by.LUC$n),1)` stations within each Land Unit. Most (i.e., `r ARUs.by.LUC[1,2]` or  `r round(ARUs.by.LUC[1,2]/nrow(sta)*100,0)`%) stations were within the `r ARUs.by.LUC[1,1]` and only (i.e., `r ARUs.by.LUC[7,2]` or  `r round(ARUs.by.LUC[7,2]/nrow(sta)*100,0)`%) within the `r ARUs.by.LUC[7,1]`. Stations were under-represented in LAR and LPR, over-represented in RDR and UPR and roughly proportional in the remaining Land Units. 
...
# Cairo(file="Output/cov.prop_hit.PNG",
#       type="png",
#       width=2200,
#       height=2000,
#       pointsize=15,
...
```{r hist LUT, echo=F, warning=F, fig.width=6.5, fig.cap="Representation of NABat stations (n=84) by Land Use Type in/out of National Parks (NP)"}

# Land Use Type
LUT.NP.hist <- sta %>% filter(NABat.Sample=="Yes") %>% group_by(NP) %>% count(Land.Use.Type, sort=TRUE)
...
```{r mean species call by year and volancy, echo=F, fig.height=8.5, fig.width=8.5, warning=FALSE}
...
fcalls.Sp <- Sp.Calls.Yr %>% filter(Classification %in% sp.to.use[sp.to.use$Sum>100,]$Classification)%>%
  ggplot(aes(x = Deployment.ID, y = Mean, fill=Volancy))+
  geom_point(colour="white", shape=21, size=4, position=position_dodge(width=0.5))+
  scale_fill_manual(values=unique(col.catVol)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  ylab(expression(paste("Mean ± 1 SE Nightly Bat Calls"))) +
  geom_linerange(aes(Deployment.ID, ymin = Mean-SE, ymax = Mean+SE), position=position_dodge(width=0.5)) +
  theme(axis.text.y = element_text(size=12), axis.title.x = element_blank())+
  theme(axis.text.x = element_text(colour = "black", size = 10)) +
  theme(legend.position = "bottom", legend.title = element_blank()) +
  facet_wrap(~Classification, ncol=3, scales="free_y")
fcalls.Sp

# group data for overall mean nightly bat calls by year withouy volancy
Sp.Calls.Yr.noVol <- call_count.Sp%>% group_by(Classification, Deployment.ID) %>% 
  summarise(Mean = mean(Count), SE = se(Count))

fcalls.Sp2 <- Sp.Calls.Yr.noVol %>% filter(Classification %in% sp.to.use[sp.to.use$Sum>100,]$Classification)%>%
  ggplot(aes(x = Deployment.ID, y = Mean, fill=Deployment.ID))+
  geom_point(colour="black", shape=21, size=4)+
  scale_fill_manual(values=unique(col.catYr)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  ylab(expression(paste("Mean ± 1 SE Nightly Bat Calls"))) +
  geom_linerange(aes(Deployment.ID, ymin = Mean-SE, ymax = Mean+SE)) +
  theme(axis.text.y = element_text(size=12), axis.title.x = element_blank())+
  theme(axis.text.x = element_text(colour = "black", size = 10)) +
  theme(legend.position = "bottom", legend.title = element_blank()) +
  facet_wrap(~Classification, ncol=3, scales="free_y")
...
# Cairo(file="Output/fcalls_Sp.PNG",
#       type="png",
#       width=2600,
#       height=2000,
#       pointsize=15,
...
See Appendix A for detailed mean nightly call counts for species /species groups by Land Unit (graphs and tables) and Natural Region (graphs).
...
knitr::include_graphics(c("./Output/activity_overlap_NP_In-Out.png",
                          "./Output/activity_overlap_NP_Pre_Volancy-In-Out.png",
                          "./Output/activity_overlap_NP_Post_Volancy_In-Out.png"))
``` 
...
ESS$Species <- sapply(lst, '[[', 2)
ESS$Covariate <- sapply(lst, '[[', 1)
ESS.tbl <- aggregate(ESS$`effectiveSize(mpost$Beta)`, by=c(list(ESS$Species)), FUN=mean)
colnames(ESS.tbl) <- c("Species","ESS")
ESS.tbl$Species.Full <- substr(ESS.tbl$Species,1,nchar(ESS.tbl$Species)-1)
ESS.tbl$Species <- word(ESS.tbl$Species,1,sep=" ")
table1 <- as.data.frame(Table.Calls)
colnames(table1) <- c("Species","Call.Count","Perc.Calls","Nightly.Calls")
table1 <- left_join(table1,ESS.tbl)
...
tmp$Group.1 <- word(tmp$Group.1,1,sep=" ")
ESS.GRSF.tbl <- left_join(table1,tmp, by=c("Species"="Group.1"))
write.csv(ESS.GRSF.tbl, "Output//ESS_GD_FINAL.csv", row.names=F )
...
# Plot Betas
# title="Heatmap depicting how species respond to predictors at the 95% CI"
par(mfrow=c(1,1))
par(mar=c(10,10,1,1))
...
Cairo(file="Output/corrplot.PNG",
      type="png",
      width=2000,
      height=2000,
      pointsize=15,
...
corrplot(toPlot[plotOrder,plotOrder], method = "color",
         ...
         title="Station Random Effect Level", mar=c(0,0,1,0), type="lower")
dev.off()
...
######################
# B estimate plots  - similar to the heatmap
#creating new data
...
colnames(newdata) <- c("Year", "Volancy_Post", "Effort", "NP_Out", 
                       "NR_Boreal","NR_Foothills","NR_Grassland","NR_Parkland",
                       "Latitude", "Longitude", 
                       "LUT_Agriculture", "LUT_Barren.Land", "LUT_Developed","LUT_Grassland","LUT_Shrubland","LUT_Water",
                       "Road.Type","Road.Distance", "Water.Type", "Water.Distance")

newdata <- newdata[,c("Year", "Volancy_Post","NP_Out", 
                      "NR_Boreal","NR_Foothills","NR_Grassland","NR_Parkland",
                      "Latitude", "Longitude", 
                      "LUT_Agriculture", "LUT_Barren.Land", "LUT_Developed","LUT_Grassland","LUT_Shrubland","LUT_Water",
                      "Road.Type","Road.Distance", "Water.Type", "Water.Distance","Effort")]


#pdf(width=9, height=8, "Output/Heatmap_95CI.pdf")
Cairo(file="Output/Heatmap_95CI.PNG",
      type="png",
      width=3400,
      height=3000,
      pointsize=15,
...
par(xpd=T)
lines(c(0.5,2.5), c(-2.6,-2.6), lty=1)
lines(c(3,15.5), c(-2.6,-2.6), lty=1)
lines(c(16,19), c(-2.6,-2.6), lty=1)

lines(c(0.5,0.5), c(-2.6,-2.4), lty=1)
lines(c(2.5,2.5), c(-2.6,-2.4), lty=1)

lines(c(3,3), c(-2.6,-2.4), lty=1)
lines(c(15.5,15.5), c(-2.6,-2.4), lty=1)

lines(c(16,16), c(-2.6,-2.4), lty=1)
lines(c(19,19), c(-2.6,-2.4), lty=1)
...
row.names(pos) <- c("Year", "Volancy_Post", "Effort", "NP_Out", 
                       "NR_Boreal","NR_Foothills","NR_Grassland","NR_Parkland",
                       "Latitude", "Longitude", 
                       "LUT_Agriculture", "LUT_Barren.Land", "LUT_Developed","LUT_Grassland","LUT_Shrubland","LUT_Water",
                       "Road.Type","Road.Distance", "Water.Type", "Water.Distance")
...
row.names(neg) <- c("Year", "Volancy_Post", "Effort", "NP_Out", 
                       "NR_Boreal","NR_Foothills","NR_Grassland","NR_Parkland",
                       "Latitude", "Longitude", 
                       "LUT_Agriculture", "LUT_Barren.Land", "LUT_Developed","LUT_Grassland","LUT_Shrubland","LUT_Water",
                       "Road.Type","Road.Distance", "Water.Type", "Water.Distance")
...
pos <- pos[c("Year", "Volancy_Post","NP_Out","NR_Boreal","NR_Foothills","NR_Grassland","NR_Parkland","Latitude", "Longitude",
                      "LUT_Agriculture", "LUT_Barren.Land", "LUT_Developed","LUT_Grassland","LUT_Shrubland","LUT_Water",
                      "Road.Type","Road.Distance", "Water.Type", "Water.Distance","Effort"),]
neg <- neg[c("Year", "Volancy_Post","NP_Out","NR_Boreal","NR_Foothills","NR_Grassland","NR_Parkland","Latitude", "Longitude",
                      "LUT_Agriculture", "LUT_Barren.Land", "LUT_Developed","LUT_Grassland","LUT_Shrubland","LUT_Water",
                      "Road.Type","Road.Distance", "Water.Type", "Water.Distance","Effort"),]
...
# m$XFormula
# XFormula = ~Year + Volancy + Effort + NP + Natural.Region + Latitude  + Longitude +
#    Land.Use.Type + Road.Type + Road.Distance + Water.Type + 
#    Water.Distance
VP = computeVariancePartitioning(m, group = c(1,1,4,2,2,2,2,2,3,3,3,3), groupnames=c("Temporal","Landscape", "Local","Effort"))
plotVariancePartitioning(m, VP = VP, las=2)
column.names <- m$spNames
row.names <- c("Temporal","Landscape", "Local","Effort", "Random")
mod.eval$SR2[order(mod.eval$SR2, decreasing=T)]

# Make a two tier plot of R2 on the top, and variance partiioning on the bottom
# pdf(width=8, height=7, "Output/Variance_Partitioning_HMSC_full.pdf")

Cairo(file="Output/Variance_Partitioning_HMSC_full.PNG",
      type="png",
      width=3400,
      height=3000,
      pointsize=15,
...
barplot(mod.eval$SR2[order(mod.eval$SR2, decreasing=T)], las=1,
        ylim=c(0,max(mod.eval$SR2)+0.1), xlim=c(0,length(column.names)+4), ylab="Pseudo R-squared")

par(mar=c(12,4,0,2))
labels <- data.frame("Species"=colnames(VP$vals[,order(mod.eval$SR2, decreasing=T)]))
# Read in the key
barplot(VP$vals[,order(mod.eval$SR2, decreasing=T)], col=viridis(n = 5, option = "D"), las=2,
        names.arg=labels$Species,
        xlim=c(0,length(column.names)+4), main="",
        ylim=c(0,1),
        ylab="Variation Partitioning")
legend(10, 1, legend=rev(row.names), fill= rev(viridis(n = 5, option = "D")))
dev.off()
############################
```
...
#### Mean annual overall nightly bat call by station.

``` {r annual mean nightly call by station, echo=F, warning=FALSE, results='asis'}
call_count$Stn.Num <- as.factor(word(call_count$Location.Name,3,sep = "\\_"))
call_count3 <- call_count %>% group_by(NP, Land.Unit.Code, GRTS.Cell.ID, Deployment.ID, Stn.Num) %>%
...
call_count4 <- call_count3 %>% 
  group_by(NP, Land.Unit.Code, GRTS.Cell.ID, Deployment.ID, Location.Name, Nights.Surveyed) %>%
  summarise(Mean=mean(Count), SE=se(Count)) %>% 
  select(NP, Land.Unit.Code, GRTS.Cell.ID, Deployment.ID, Location.Name, Nights.Surveyed, Mean, SE)

call_count4$Mean.SE <- paste0(round(call_count4$Mean,2)," ± ",round(call_count4$SE,2))
call_count4 <- call_count4 %>% select(-c(Mean, SE))

stn.count.year <- call_count4  %>% pivot_wider(names_from = c("Deployment.ID"),  values_from = c("Mean.SE","Nights.Surveyed"))

stn.count.year <- stn.count.year[,c("NP","Land.Unit.Code","GRTS.Cell.ID","Location.Name","Mean.SE_2015","Nights.Surveyed_2015",     
                                    "Mean.SE_2016","Nights.Surveyed_2016","Mean.SE_2017","Nights.Surveyed_2017","Mean.SE_2018","Nights.Surveyed_2018", "Mean.SE_2019","Nights.Surveyed_2019","Mean.SE_2020","Nights.Surveyed_2020"  )]
colnames(stn.count.year) <- c("NP","Land Unit","GRTS ID","Station", "Mean 2015", "# Nights 2015","Mean 2016", "# Nights 2016",
                               "Mean 2017", "# Nights 2017","Mean 2018", "# Nights 2018","Mean 2019", "# Nights 2019",
                               "Mean 2020", "# Nights 2020")


opts <- options(knitr.kable.NA = "")
print(knitr::kable(stn.count.year,
             caption = "Mean ± 1 SE nightly bat calls for each NABat station. # Nights refers to the number of nights the station was surveyed.",
             align="llllrrrrrrrrrrrr"))
  options(opts)
...
#### Mean species / species group calls for each year and volancy period by Natural Region (graphs)

``` {r natural region annual and volancy species graphs, warnings= F, echo=F, fig.height=8.5, fig.width=8.5}

# group data for NR mean nightly bat calls by year and volancy
NR.Calls.Yr <- call_count.Sp%>% group_by(Classification, Deployment.ID, Natural.Region, Volancy) %>% 
  summarise(Mean = mean(Count), SE = se(Count))

NR="Boreal"
NR.Calls.Yr.fn <- function(NR=NR){
fcalls.NR2 <- NR.Calls.Yr %>% filter(Natural.Region==NR) %>%
  filter(Classification %in% sp.to.use[sp.to.use$Sum>100,]$Classification)%>%
  ggplot(aes(x = Deployment.ID, y = Mean, fill=Volancy))+
  geom_point(colour="white", shape=21, size=4, position=position_dodge(width=0.5))+
  scale_fill_manual(values=unique(col.catVol)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
  ...
  theme(axis.text.x = element_text(colour = "black", size = 10)) +
  theme(legend.position = "bottom", legend.title = element_blank()) +
  facet_wrap(~Classification+Natural.Region, ncol=3, scales="free_y")
fcalls.NR2
}

par(mfrow=c(1,2), las=1)
NR.Calls.Yr.BO <- NR.Calls.Yr.fn(NR="Boreal") ; NR.Calls.Yr.BO
NR.Calls.Yr.FO <- NR.Calls.Yr.fn(NR="Foothills") ; NR.Calls.Yr.FO
NR.Calls.Yr.GR <- NR.Calls.Yr.fn(NR="Grassland") ; NR.Calls.Yr.GR
NR.Calls.Yr.PA <- NR.Calls.Yr.fn(NR="Parkland") ; NR.Calls.Yr.PA
NR.Calls.Yr.RM <- NR.Calls.Yr.fn(NR="Rocky Mountain") ; NR.Calls.Yr.RM


```

#### Mean species / species group calls for each year and volancy period by Land Unit (tables)
...
knitr::include_graphics(c("./Output/activity_overlap_NR_Boreal.png",
                          "./Output/activity_overlap_NR_Foothills.png",
                          "./Output/activity_overlap_NR_Grassland.png",
                          "./Output/activity_overlap_NR_Parkland.png",
                          "./Output/activity_overlap_NR_RockyMountain.png"))
``` 
...
knitr::include_graphics(c("./Output/activity_overlap_LUT_Agriculture.png",
                          "./Output/activity_overlap_LUT_BarrenLand.png",
                          "./Output/activity_overlap_LUT_Developed.png",
                          "./Output/activity_overlap_LUT_Forest.png",
                          "./Output/activity_overlap_LUT_Grassland.png",
                          "./Output/activity_overlap_LUT_Shrubland.png",
                          "./Output/activity_overlap_LUT_Water.png"))
``` 
...
```{r sta covariate table,  cap= "Station Attributes", echo=F, warning=FALSE}
names(sta)
names(sta2)

sta.table <- sta2 %>% arrange(NP, Land.Unit.Code, GRTS.Cell.ID, Location.Name) %>% 
  group_by(NP, GRTS.Cell.ID, Location.Name) %>% select(GRTS.Cell.ID, Location.Name, Orig.Name, NP, Land.Unit.Code, Yrs.Surveyed, Natural.Region, Land.Use.Type, Human.Footprint.Sublayer, Human.Footprint.Type, Human.Footprint.Distance, Road.Type.Ord, Road.Type, Road.Distance, Water.Type.Ord, Water.Type, Water.Distance)
sta.table <- sta.table[c(2:17)]
...
opts <- options(knitr.kable.NA = "--")
knitr::kable(sta.table, 
             caption = "Station Attributes and HMSC Covariates", 
             digits=0)
options(opts)
...
```{r HMSC covariate table,  cap= "HMSC model inputs", echo=F, warning=FALSE}
