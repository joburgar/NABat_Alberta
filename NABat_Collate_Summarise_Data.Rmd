...
nrow(week.obs)
# week.obs <- week.obs %>% filter(!(Location.Name %in% ABMI.sites & Week==25 | 
#                                            Location.Name %in% ABMI.sites & Week==26))
week.obs <- week.obs %>% filter(!Location.Name %in% FortChip.sites)

nrow(daily.obs %>% filter(Location.Name %in% ABMI.sites))
...
# Make the Y Matrix
Y <- week.obs[c(6:13)]
row.names(Y) <- week.obs$Location.Name_Yr_Week
Y <- as.matrix(Y)
...
names(sta2)
head(sta.week)
sta.week <- left_join(sta2 %>% select("Location.Name","NP","Natural.Region","Land.Unit.Code","Land.Use.Type","Latitude","Longitude","Road.Type.Ord","Road.Dist.HMSC","Road.Inf","Water.Type.Ord","Water.Dist.HMSC","Water.Inf","HF.Dist.HMSC"), sta.week)
XData <- data.frame(sta.week)

as.data.frame(XData %>% filter(Land.Unit.Code=="BNP") %>% group_by(Location.Name) %>% count(Year))
...
XData <- XData %>% filter(!Location.Name %in% ABMI.sites) %>% filter(!Location.Name %in% FortChip.sites)
nrow(XData)
...
colnames(ESS.tbl.full)[4:7] <- c("GRSF.Mean_subweather","GSRF.Max_subweather","GRSF.Mean_suball", "GRSF.Max_suball" )
# write.csv(ESS.tbl, "Output//ESS_GD_msubweather2.csv", row.names=F )
...
colnames(newdata) <- c("Year", "Volancy_Post", "Effort", "NP_Out", 
                       "NR_Boreal", "NR_Canadian.Shield","NR_Foothills","NR_Grassland","NR_Parkland",
                       "Latitude", "Longitude", 
...
pos <-  pos[,plotOrder]
row.names(pos) <- colnames(newdata)

neg  <- postBeta$supportNeg[2:nrow(postBeta$supportNeg),]
neg  <- neg[,plotOrder]
row.names(neg) <- colnames(newdata)
...
# m$XFormula
# XFormula = ~Year + Volancy + Effort + NP + Natural.Region + Latitude  + 
#    Land.Use.Type + Road.Type + Road.Distance + Water.Type + 
#    Water.Distance + Max.Temp + Max.Hum
...
library(viridis)
pdf(width=8, height=7, "Output/Variance_Partitioning_HMSC_full_model.pdf")
...
call_count %>% filter(Land.Unit.Code=="BNP") %>% group_by(Deployment.ID)%>% count(Location.Name)
