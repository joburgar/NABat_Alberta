...
#Load Packages
list.of.packages <- c("data.table", "leaflet", "tidyverse", "lunar", "zoo", "colortools", "mapview", "fs", "lubridate", "camtrapR", "circular", "Hmsc", "corrplot", "plotrix", "RColorBrewer", "MuMIn")
...
sta2 <- sta %>% filter(Land.Unit.Code!="EINP") # now only 113 stations
nrow(sta2) # EINP has been removed

plot(sta2$Longitude , sta2$Latitude, asp=1)
...
as.data.frame(no.weather.df)
no.weather.df$Year <- word(no.weather.df$Location.Name.Year, 4, sep="_")
as.data.frame(no.weather.df %>% group_by(Year,GRTS.Cell.ID)%>% count(Orig.Name))
as.data.frame(no.weather.df %>% ungroup()%>% count(Year)%>% ungroup())
8/66 # 2015 = 12%
9/77 # 2016 = 12%
9/56 # 2017 = 16% 
11/81 # 2018 = 14% (Edson, Switzer, Lost River, Medicine Hat)
4/108 # 2019 = 4%, 
84/168 # 2020 = 50%, mostly new sites (UPR incl ABMI)

# will remove 137 survey weeks of data if want to include max temp and max hum
# will remove 37 stations as weather data limiting based on station
# try as a "sub-sample" to see if it changes the output for this data set
# also do regular analysis for "sub-sample" to see if general trends hold
...
###--- need to delete the ABMI and Fort Chip sites as recordings are very low so perhaps issues with detectors
ABMI.sites <- as.matrix(sta %>% filter(grepl("NABAT",Orig.Name)) %>% select(Location.Name))
FortChip.sites <- as.matrix(sta %>% filter(grepl("Chip",Orig.Name)) %>% select(Location.Name))
nrow(week.obs %>% filter(Location.Name %in% ABMI.sites))

week.obs <- week.obs %>% filter(!Location.Name %in% FortChip.sites)
nrow(daily.obs %>% filter(Location.Name %in% FortChip.sites))
...
HMSC.stn.yr.week <- unique(week.obs$Location.Name_Yr_Week) 

# ABMI.sites.remove.dates <- seq.Date(as.Date("2020-06-14"),as.Date("2020-07-02"), by="1 day")
# daily.obs <- daily.obs %>% filter(!(Location.Name %in% ABMI.sites & Date %in% ABMI.sites.remove.dates))
daily.obs <- daily.obs %>% filter(!Location.Name %in% ABMI.sites)
daily.obs <- daily.obs %>% filter(!Location.Name %in% FortChip.sites)
daily.obs %>% filter(Location.Name %in% ABMI.sites)
...
# Make the Y Matrix
Y <- week.obs[c(6:13)]
row.names(Y) <- as.data.frame(week.obs$Location.Name_Yr_Week)
Y <- as.matrix(Y)
...
names(sta2)
nrow(sta.week)
sta.week <- left_join(sta.week, sta2 %>% select("Location.Name","NP","Natural.Region","Land.Unit.Code","Land.Use.Type","Latitude","Longitude","Road.Type.Ord","Road.Dist.HMSC","Road.Inf","Water.Type.Ord","Water.Dist.HMSC","Water.Inf","HF.Dist.HMSC"))
...
XData$Road.Inf <- stdize(XData$Road.Inf)
XData$Water.Type <- stdize(XData$Water.Type.Ord)
XData$Water.Distance <- stdize(XData$Water.Dist.HMSC)
XData$Water.Inf <- stdize(XData$Water.Inf)
XData$Human.Footprint.Distance <- stdize(XData$HF.Dist.HMSC)

summary(XData)
nrow(XData)

# # subset using the weather data
# XData.sub <- left_join(XData, weekly.max.env.cov %>% select(-Max.Wind, -Week), by="Location.Name_Yr_Week") 
# XData.sub <- XData.sub[complete.cases(XData.sub),] # removed 116 data points ~20%
# summary(XData.sub)
# #standardize ordinal/continuous variables
# XData.sub$Max.Temp <- stdize(XData.sub$Max.Temp)
# XData.sub$Max.Hum <- stdize(XData.sub$Max.Hum)
# 
# sub.data <- XData.sub$Location.Name_Yr_Week
# Y.sub <- Y[sub.data,]
# nrow(Y.sub)
# nrow(XData.sub)
...
corrplot.mixed(tmp) # 0.83 correlation between Road.Distance and Human.Footprint.Distance
...
###--- SUB DATASET WITH MODEL 1 and WITH WEATHER TERMS 
# Add a station-level random effect (for the covariances)
studyDesign.sub = data.frame(station = as.character(XData.sub$Location.Name))

rL.sub = HmscRandomLevel(units = studyDesign.sub$station)
# Added poisson distributed

# sub model WITHOUT weather terms
m.sub.all = Hmsc(Y = Y.sub, XData = XData.sub, XFormula = ~Year + Week + Effort +
           NP + Natural.Region + Latitude + Longitude + Land.Use.Type +
           Road.Type + Road.Distance + Water.Type + Water.Distance,
         studyDesign = studyDesign.sub, ranLevels = list(station = rL), distr="poisson")

m.sub.all = sampleMcmc(m.sub.all, thin = thin, samples = samples, transient = transient,
               nChains = nChains, verbose = verbose, nParallel = 3)

# sub model WITH weather terms
m.sub.weather2 = Hmsc(Y = Y.sub, XData = XData.sub, XFormula = ~Year + Week + Effort +
           NP + Natural.Region + Latitude + Land.Use.Type +
           Road.Type + Road.Distance + Water.Type + Water.Distance + Max.Temp + Max.Hum,
         studyDesign = studyDesign.sub, ranLevels = list(station = rL), distr="poisson")

m.sub.weather2 = sampleMcmc(m.sub.weather2, thin = thin, samples = samples, transient = transient,
               nChains = nChains, verbose = verbose, nParallel = 3)

saveRDS(m.sub.weather2,file="Output/MultiSpeciesHMSC_weather2.RData")
saveRDS(m.sub.all,file="Output/MultiSpeciesHMSC_sub.RData")
saveRDS(m,file="Output/MultiSpeciesHMSC.RData")
...
# group data for overall mean nightly bat calls by year
...
fcalls.Sp2 <- Sp.Calls.Yr %>% filter(Classification %in% sp.to.use[sp.to.use$Sum>100,]$Classification)%>%
...
# m <- readRDS("Output/MultiSpeciesHMSC_weather2.RData")
# m <- readRDS("Output/MultiSpeciesHMSC_sub.RData")
# m <- readRDS("Output/MultiSpeciesHMSC.RData")
...
ESS.tbl <- left_join(ESS.tbl,tmp, by=c("Species"="Group.1"))
write.csv(ESS.tbl, "Output//ESS_GD_FINAL.csv", row.names=F )
...
``` {r natural region annual and volancy species graphs, echo=F, fig.height=8.5, fig.width=6.5}
...
NR.Calls.Yr.BO <- NR.Calls.Yr.fn(Natural.Region="Boreal")
...
#### Mean species / species group calls for each year and volancy period by Land Unit (graphs)
